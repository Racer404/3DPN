...
gt_dir   = "room/images_400"
pred_dir = "room/trained/INF_res=7_dStep=113_raypass_mae.8+ssim.2/epochs/63"
...
for fname in tqdm(filenames):
    gt   = load_image(os.path.join(gt_dir, fname))
    pred = load_image(os.path.join(pred_dir, fname))

    assert gt.shape == pred.shape, f"Shape mismatch: {fname}"

    psnr_list.append(psnr_torch(pred, gt).item())
    ssim_list.append(ssim_torch(pred, gt).item())
    lpips_list.append(lpips_torch_alex(pred, gt))
